...
class RealtimeSyncService {
  ...
  final FirebaseDatabase _db = FirebaseDatabase.instanceFor(
    ...
    databaseURL:
        'https://travelapp-ab318-default-rtdb.asia-southeast1.firebasedatabase.app',
  );
  ...
  final _repliesController =
      StreamController<MapEntry<int, List<ReplyModel>>>.broadcast();
  final _reactionsController =
      StreamController<MapEntry<int, Map<String, int>>>.broadcast();
  final _notificationsController =
      StreamController<AppNotificationModel>.broadcast();

  Stream<ReviewModel> get reviewsStream => _reviewsController.stream;
  Stream<MapEntry<int, List<ReplyModel>>> get repliesStream =>
      _repliesController.stream;
  Stream<MapEntry<int, Map<String, int>>> get reactionsStream =>
      _reactionsController.stream;
  Stream<AppNotificationModel> get notificationsStream =>
  ...
  List<ReplyModel> getRepliesFor(int reviewId) => _repliesCache[reviewId] ?? [];
  ...
  void seedReviewMeta({
    ...
    final reactionCounts = {'likes': likes, 'dislikes': dislikes};
    _reactionsCache[reviewId] = reactionCounts;
    _repliesCache[reviewId] = replies;
    _reactionsController.add(MapEntry(reviewId, reactionCounts));
        ...
        .listen((event) async {
          if (event.snapshot.value != null) {
            ...
            if (currentUserId == null) {
              ...
              currentUserId = session?['uid'] as String?;
            }

            if (notif.userId.isNotEmpty && notif.userId != currentUserId) {
              return;
            }
  ...
  void startDestinationSync(int destinationId) {
    ...
    if (!_subs.containsKey(reviewsKey)) {
      _subs[reviewsKey] = _db
          .ref('reviews_realtime/$destinationId')
          .onChildAdded
          .listen((event) {
            if (event.snapshot.value != null) {
              final data = event.snapshot.value as Map<dynamic, dynamic>;
  ...
  void listenToReviewMeta(int reviewId) {
    ...
    _subs[likesKey] = _db.ref('review_likes/$reviewId').onValue.listen((event) {
      ...
      } else if (_reactionsCache.containsKey(reviewId)) {
        final cached = _reactionsCache[reviewId]!;
    ...
    _subs[repliesKey] = _db.ref('replies/$reviewId').onValue.listen((event) {
      ...
      if (event.snapshot.value != null) {
        for (final child in event.snapshot.children) {
          replies.add(
            ReplyModel.fromFirebase(
              Map<dynamic, dynamic>.from(child.value as Map),
            ),
          );
        }
      } else if (_repliesCache.containsKey(reviewId)) {
        replies = _repliesCache[reviewId]!;
      }
      _repliesCache[reviewId] = replies;
      _repliesController.add(MapEntry(reviewId, replies));
    });
